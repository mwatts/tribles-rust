...
fn gen_var_conversion(crate_path: &syn::Path, binding: &Ident, v: &FindVariable) -> TokenStream2 {
    ...
    if v.fallible {
        if let Some(ref ty) = v.ty {
            quote! {
                let #name: ::core::result::Result<#ty, _> =
                    #crate_path::inline::TryFromInline::try_from_inline(#name.extract(#binding));
            }
        } else {
            quote! {
                let #name =
        ...
        if let Some(ref ty) = v.ty {
            quote! {
                let #name: #ty = match #crate_path::inline::TryFromInline::try_from_inline(
            ...
            quote! {
                let #name = match #crate_path::inline::TryFromInline::try_from_inline(
                    #name.extract(#binding)
                ) {
                    ::core::result::Result::Ok(__v) => __v,
                    ::core::result::Result::Err(_) => return ::core::option::Option::None,

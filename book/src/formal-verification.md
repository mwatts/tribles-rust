...
| Collection algebra (`src/collection/*.rs`) | Every directly materializable encoding join is associative, commutative, and idempotent; record insertion is idempotent; typed covers preserve exact payload support; a bound `CollectionMapping` is a join homomorphism; substituting an intrinsic mapping-entity id with an extrinsic one does not change meaning; warm attach/ensure executes zero joins or mappings; every successful computation is persisted even if later planning fails; only Yard/GC removes materializations; unsigned `MERGE`/`DERIVE` records never manufacture authority. | Add bounded record-set generators plus deterministic simulations that compare every accepted `Cover<E>` and cover-aware logical view with direct source reconstruction, inject late failures after successful work, and instrument warm paths to prove zero algebra. Keep direct reconstruction as an offline oracle rather than a production read step. |

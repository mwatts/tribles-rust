...
| Collection algebra (`src/collection/*.rs`) | Every `CollectionEncoding` validates canonical members and has an associative, commutative, idempotent join; record insertion is idempotent; valid typed covers preserve exact payload support; a bound `CollectionMapping` validates the target's declared algorithm and parameters and is a join homomorphism; substituting an intrinsic mapping-entity id with an extrinsic one does not change meaning; unsigned `MERGE`/`DERIVE` or mapping evidence never manufactures authority. | Add bounded record-set generators plus deterministic simulations that compare every accepted `Cover<E>` and cover-aware logical view with direct source reconstruction. |

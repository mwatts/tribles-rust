...
| Collection algebra (`src/collection/*.rs`) | Every `CollectionEncoding` validates canonical typed blob members and any directly materializable join it exposes is associative, commutative, and idempotent; record insertion is idempotent; valid typed covers preserve exact payload support; a bound `CollectionMapping` validates the target's declared algorithm and parameters and is a join homomorphism; substituting an intrinsic mapping-entity id with an extrinsic one does not change meaning; source-bound derivation stores its selected source before its target; unsigned `MERGE`/`DERIVE` records never manufacture authority. | Add bounded record-set generators plus deterministic simulations that compare every accepted `Cover<E>` and cover-aware logical view with direct source reconstruction, including missing source-bound children and encodings with no direct physical join. |

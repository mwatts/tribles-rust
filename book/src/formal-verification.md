...
| Collection algebra (`src/collection/*.rs`) | Every directly materializable encoding join is associative, commutative, and idempotent; record insertion is idempotent; typed covers preserve exact payload support; a bound `CollectionMapping` is a join homomorphism; substituting an intrinsic mapping-entity id with an extrinsic one does not change meaning; read-only attach and repeated ensure over a resident exact target result execute zero joins or mappings, while capacity/unsupported attempts may be retried because negative results are not durable; exact-derived maintenance is deterministic by target size tier, prefers two resident target children over crossing the mapping, derives a resident corresponding source node before descending, descends through capacity-terminal source nodes, and never synthesizes a missing source merge; every successful `MERGE` or `DERIVE` is persisted even if later planning fails; only Yard/GC removes materializations; unsigned records never manufacture authority. | Add bounded record-set generators plus deterministic simulations over resident target results, complete and partial target children, resident and absent corresponding source nodes, source-leaf fallbacks, and capacity-terminal nodes. Compare every accepted `Cover<E>` and cover-aware logical view with direct source reconstruction, permute record insertion order, inject late failures after successful work, assert zero source merges, and instrument warm paths to prove zero algebra. Keep direct reconstruction as an offline oracle rather than a production read step. |

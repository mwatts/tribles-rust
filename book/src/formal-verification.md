...
| Collection algebra (`src/collection/*.rs`) | Every representable member join is associative, commutative, and idempotent, while finer covers preserve the total logical join across capacity boundaries; record insertion is idempotent; typed covers preserve exact payload support; a bound `CollectionMapping` is a join homomorphism; substituting an intrinsic mapping-entity id with an extrinsic one does not change meaning; read-only attach and repeated ensure over a resident exact target result execute zero joins or mappings, while capacity attempts may be retried because negative results are not durable; exact-derived maintenance is deterministic by target size tier, prefers two resident target children over crossing the mapping, derives a resident corresponding source node before descending, descends through capacity-terminal source nodes, and synthesizes a source merge only when a target join names that exact immutable representation dependency; every successful `MERGE` or `DERIVE` is persisted even if later planning fails; only Yard/GC removes materializations; unsigned records never manufacture authority. | Add bounded record-set generators plus deterministic simulations over resident target results, complete and partial target children, resident and absent corresponding source nodes, source-leaf fallbacks, capacity-terminal nodes, and explicitly missing join dependencies. Compare every accepted `Cover<E>` and cover-aware logical view with direct source reconstruction, permute record insertion order, inject late failures after successful work, assert that no unrequested source merge is synthesized, and instrument warm paths to prove zero algebra. Keep direct reconstruction as an offline oracle rather than a production read step. |

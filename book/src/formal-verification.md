...
| Collection algebra (`src/collection/*.rs`) | Every representable member join is associative, commutative, and idempotent, while finer covers preserve the total logical join across capacity boundaries; record insertion is idempotent; typed covers preserve exact payload support; a bound `CollectionMapping` is a join homomorphism; substituting an intrinsic mapping-entity id with an extrinsic one does not change meaning; read-only attachment executes no joins or mappings; `ensure` publishes only missing `DERIVE` work; `maintain` additionally reaches a deterministic target size-tier normal form and synthesizes a source merge only when a selected target join names that exact immutable representation dependency; every successful `MERGE` or `DERIVE` is persisted even if later work fails; store-level ensure and maintain operations return a fresh `CollectionSnapshot` which keeps the observation with its frozen support and realized cover; only Yard/GC removes materializations; unsigned records never manufacture authority. | Add bounded record-set generators plus deterministic simulations over resident target results, support-equivalent physical covers, capacity-terminal nodes, and explicitly missing join dependencies. Compare every accepted `Cover<E>` and snapshot-owned logical view with direct source reconstruction, permute record insertion order, inject late failures after successful work, assert that ensure emits no `MERGE`, assert deterministic tiered maintenance, and instrument warm paths to prove zero algebra. Keep direct reconstruction as an offline oracle rather than a production read step. |

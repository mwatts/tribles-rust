...
> **UFOID** = *Universal Forgettable Ordered ID* — a 128-bit
> identifier built from a 32-bit time prefix (the low 32 bits of the
> UNIX time in milliseconds) plus 96 random bits. The prefix gives
> recently-minted ids high locality: they sort roughly by mint time,
> so a system can range-scan "everything created in the last N hours"
> directly off the id. *Forgettable* refers to that prefix wrapping
> around about every 50 days — the high bits of absolute time are
> intentionally dropped, leaving a rolling window rather than a
> monotonic-forever clock. Within the window the ordering is exact;
> outside it the prefix repeats. That's the right trade for systems
> that only reason over a bounded recent horizon and garbage-collect
> older data — caches, robotics pipelines, event streams — where the
> time-locality is worth far more than a globally unique timestamp.
> (Use [`timestamp_distance`](triblespace::core::id::ufoid::timestamp_distance)
> to compare two prefixes correctly across the rollover.)
>
> **FUCID** = *Fast Unsafe Compressible ID* — a 128-bit identifier
> generated by XOR-ing a per-source salt with a monotonic counter, so
> consecutive ids from the same source compress extremely well at the
> cost of being predictable per source. See the
> [Comparison of Identifier Types](#comparison-of-identifier-types)
> table below for when to reach for which.
...
[`Hash`](triblespace::core::inline::encodings::hash::Hash) and
[`Handle`](triblespace::core::inline::encodings::hash::Handle).  
...
[`Hash`](triblespace::core::inline::encodings::hash::Hash).
...
[`TryFromInline`](triblespace::core::inline::TryFromInline) implementation for `ExclusiveId`.

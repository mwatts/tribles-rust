...
/// `Head::new_local_leaf` and the owner joins the PATCH's exact persistent
/// root owner set. Trie shape and owner identity are independent.
///
/// The owner is borrowed (not owned) so the ingest hot loop pays
/// **zero** atomic ref-count traffic per trible — the receiving PATCH clones
/// an owner only when that allocation first joins its set. The caller

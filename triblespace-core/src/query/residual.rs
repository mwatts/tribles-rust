...
struct FiniteFormulaNode {
    ...
    /// Outer residual leaf occurrence whose compiled tree owns this node.
    ///
    /// Formula cursors no longer repeat this value in their exit identity:
    /// every node is allocated for exactly one outer occurrence, so the
    /// structural program is the canonical source of ownership.
    owner: usize,
...
impl FiniteFormulaProgram {
    fn compile<'a>(root: &dyn Constraint<'a>, leaves: &[ResidualLeaf]) -> Self {
        ...
        impl Builder {
            ...
            fn compile_node<'a>(
                ...
                let kind = if let Some(children) = constraint.residual_union_children() {
                    ...
                    self.compile_or_children(constraint, path, owner, &mut compiled);
            ...
            fn compile_or_children<'a>(
                ...
                for child in 0..children.len() {
                    ...
                    if constraint.residual_union_children().is_some() {
                        self.compile_or_children(constraint, path, owner, compiled);
                    } else {
        ...
        for (occurrence, leaf) in leaves.iter().enumerate() {
            ...
            roots[occurrence] = Some(builder.compile_node(constraint, &mut Vec::new(), occurrence));
        }
        Self {
            nodes: builder
                .nodes
    ...
    fn owner(&self, id: FormulaNodeId) -> usize {
        self.node(id).owner
    }
    ...
    fn start(
        ...
        FormulaProgramCounter {
            ...
            returns: Box::new([]),
            exit: CandidateExit::from_formula(variable, self.roots.len(), occurrence, verb, true),
        }
    }
    ...
    fn interned_proposal_streamability(
        ...
        let focused = match formula_pcs.get(cursor.pc).focus {
            FormulaFocus::Action {
                node,
...
/// Immediate consumer of one completed candidate-producing Formula node.
///
/// Nested destinations are captured on the persistent return edge when the
/// child is selected. Root is represented by the absence of such an edge.
#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
...
impl FormulaFocus {
    fn node(&self) -> FormulaNodeId {
        match self {
            Self::Action { node, .. } | Self::Plan { node, .. } | Self::Complete { node, .. } => {
                *node
            }
        }
    }
...
/// Exact ordinary Candidate future after one Formula root completes.
///
/// This is the normal form of both proposal and confirmation entry histories:
/// the variable, relevant occurrences, and already-checked occurrences are
/// precisely the fields that the successor Candidate state will own.
#[derive(Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
...
/// Query-local canonical name for the exact Candidate future shared by every
/// Formula state in one activation family.
#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
struct CandidateExitId(u32);

/// Exact Formula execution identity.
///
/// Structural progress is interned independently from the outer WCO future so
/// two activations with the same Formula control share one persistent PC
/// spine without becoming the same residual state.
#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
struct FormulaCursor {
    pc: FormulaPcId,
    exit: CandidateExitId,
}

impl FormulaCursor {
    fn with_pc(self, pc: FormulaPcId) -> Self {
        Self { pc, ..self }
    }
}
...
/// focus, done mask, and outer return edge, so a child transition never copies
/// or hashes the historical stack. The outer WCO future is deliberately not
/// part of this structural spine.
#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
struct FormulaReturnRecord {
    kind: FormulaReturnKind,
    parent: FormulaPcId,
    child: usize,
    destination: Option<FormulaResultDestination>,
}

/// Exact O(1)-spine key stored once in the query-local PC arena. `focus` may
/// contain the current connective's dynamic child mask, while ancestry is a
/// compact canonical ID. The outer WCO continuation lives beside this record
/// in `FormulaCursor`.
#[derive(Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
...
impl ResidualPlan {
    ...
    fn compile_production<'a>(root: &dyn Constraint<'a>) -> Self {
        fn visit<'a>(
            ...
            match constraint.residual_shape() {
                ...
                ConstraintShape::Opaque => {
                    let lowering = if constraint.residual_union_children().is_some() {
    ...
    fn interned_formula_proposal_streamability(
        ...
        let owner = self
            .finite_formula
            .owner(formula_pcs.get(cursor.pc).focus.node());
        if !exit.checked.contains(owner) {
            return FormulaProposalStreamability::Barrier(
    ...
    fn execution_proposal_coverage_from_ordinary<'a>(
        ...
        if ordinary == ProposalCoverage::Exact {
            return ordinary;
        }
        ...
        match select_program(constraint, request) {
            Some(_) => ordinary.max(constraint.residual_program_proposal_coverage(variable, bound)),
            None => ordinary,
        }
    }
...
pub struct ResidualStateStats {
    ...
    /// ledger and committed directly through a Terminal continuation.
    ///
...
impl CandidateExit {
    fn from_formula(
        ...
        assert!(
            occurrence < leaf_count,
            "Formula owner is not a leaf occurrence"
        );
        match verb {
            UnionVerb::Propose { relevant } => {
                assert!(
                    relevant.is_valid_for(leaf_count) && relevant.contains(occurrence),
                    "Formula proposer is not relevant"
                );
            ...
            UnionVerb::Confirm {
                ...
                assert!(
                    relevant.is_valid_for(leaf_count)
                        ...
                        && relevant.contains(occurrence)
                        && !checked.contains(occurrence),
                    "Formula confirmer is not an unchecked relevant occurrence"
                );
                checked.insert(occurrence);
                Self {
                    variable,
    ...
    fn validate(&self, leaf_count: usize, formula_pcs: &FormulaPcInterner) {
        ...
        match &self.phase {
            ...
            ResidualPhase::Formula { cursor } => {
                let exit = formula_pcs.candidate_exit(*cursor);
                validate_variable(exit.variable);
                validate_sets(&exit.relevant, &exit.checked);
            }
        }
    }
...
struct FormulaPcInterner {
    ...
    candidate_exits: IndexSet<CandidateExit, ahash::RandomState>,
...
impl FormulaPcInterner {
    fn intern_candidate_exit(&mut self, exit: CandidateExit) -> CandidateExitId {
        if self.candidate_exits.len() > u32::MAX as usize {
            if let Some(raw) = self.candidate_exits.get_index_of(&exit) {
                return CandidateExitId(raw as u32);
            }
            panic!("too many residual formula Candidate exits");
        }
        let (raw, _) = self.candidate_exits.insert_full(exit);
        CandidateExitId(u32::try_from(raw).expect("too many residual formula Candidate exits"))
    }
    ...
    fn start_with_proposer_checked(
        ...
        let exit = self.intern_candidate_exit(CandidateExit::from_formula(
            variable,
            program.roots.len(),
            occurrence,
            verb,
    ...
    fn select_child_with(
        ...
        let return_to = self.intern_return(FormulaReturnRecord {
            ...
            destination: match kind {
                FormulaReturnKind::Guard => None,
                FormulaReturnKind::Child => Some(match program.node(node).kind {
                    FiniteFormulaNodeKind::And { .. } => FormulaResultDestination::ParentAnd,
                    FiniteFormulaNodeKind::Or { .. } => FormulaResultDestination::ParentOr,
                    FiniteFormulaNodeKind::Atom => {
                        panic!("a Formula child return edge named an Atom parent")
                    }
                }),
            },
        });
    ...
    fn skip_child(
        ...
        let (node, stage, done, return_to, grade) = {
            let counter_record = self.get(counter);
            let FormulaFocus::Plan { node, stage, done } = &counter_record.focus else {
                panic!("only a residual formula Plan can skip a child")
            };
    ...
    fn complete(&mut self, program: &FiniteFormulaProgram, counter: FormulaPcId) -> FormulaPcId {
        let (node, stage, return_to, grade) = {
            ...
            (node, stage, counter_record.return_to, self.grade(counter))
        };
        self.intern_record(
            FormulaPcRecord {
                focus: FormulaFocus::Complete { node, stage },
    ...
    fn resume_completed(
        ...
        program: &FiniteFormulaProgram,
        cursor: FormulaCursor,
    ) -> Result<InternedFormulaSuccessor, CandidateExitId> {
        let (completed, completed_stage, return_to, grade) = {
            let counter_record = self.get(cursor.pc);
            let FormulaFocus::Complete { node, stage } = counter_record.focus else {
                panic!("only a completed residual formula can return")
            };
            (node, stage, counter_record.return_to, self.grade(cursor.pc))
        };
        ...
        let (parent_node, parent_stage, parent_done, parent_return) = {
            ...
            (*node, *stage, done.clone(), parent.return_to)
        };
        let children = program
            .node(parent_node)
            .children()
        ...
        Ok(InternedFormulaSuccessor::Formula(FormulaCursor {
            pc: parent,
            exit: cursor.exit,
        }))
    }
    ...
    fn candidate_exit_by_id(&self, id: CandidateExitId) -> &CandidateExit {
        self.candidate_exits
            .get_index(id.0 as usize)
            .expect("interned residual formula Candidate exit exists")
    }

    fn candidate_exit(&self, cursor: FormulaCursor) -> &CandidateExit {
        self.candidate_exit_by_id(cursor.exit)
    }
    ...
    fn result_destination(&self, counter: FormulaPcId) -> FormulaResultDestination {
        ...
        address
            .destination
            .expect("a candidate return edge lost its structural destination")
    }
    ...
    #[cfg(test)]
    fn candidate_exit_len(&self) -> usize {
        self.candidate_exits.len()
    }
    ...
    #[cfg(test)]
    fn materialize(&self, cursor: FormulaCursor) -> FormulaProgramCounter {
        ...
        FormulaProgramCounter {
            ...
            returns: returns.into_boxed_slice(),
            exit: self.candidate_exit(cursor).clone(),
        }
    }
}
...
impl StateInterner {
    ...
    fn formula(&self, cursor: FormulaCursor) -> &FormulaPcRecord {
        self.formula_pcs.get(cursor.pc)
    }

    fn formula_candidate_exit(&self, cursor: FormulaCursor) -> &CandidateExit {
        self.formula_pcs.candidate_exit(cursor)
    }
...
/// Phase-indexed live Formula payload.
///
/// The vector contains one cell per open OR reducer, followed by at most one
/// `Current` cell while an affine candidate stream actually exists. AND entry
/// and return allocate no cell. In particular, planning, Support, action
/// execution, OR admission, and OR emission may carry no `Current` at all.
#[derive(Clone, Debug)]
enum FormulaLiveCell {
    Current(CandidatePayload),
    Or {
        /// Immutable source shared by every supported OR child.
        source: CandidatePayload,
        /// Activation-private ordered SET accumulated across completed arms.
        accumulator: FormulaOrAccumulator,
    },
}

impl FormulaLiveCell {
    fn empty_like(&self, parent_count: usize) -> Self {
        match self {
            Self::Current(_) => Self::Current(CandidatePayload::empty(parent_count)),
            Self::Or { .. } => Self::Or {
    ...
    fn take_parent_tail(&mut self, first: usize, parent_count: usize) -> Self {
        match self {
            Self::Current(current) => Self::Current(current.take_parent_tail(first, parent_count)),
            Self::Or {
                source,
                accumulator,
            } => Self::Or {
                source: source.take_parent_tail(first, parent_count),
                accumulator: accumulator.take_tail(first),
            },
        }
    }

    fn normalize_and_defer_source(&mut self, parent_count: usize) {
        match self {
            Self::Current(current) => current.normalize_for(parent_count),
            Self::Or { source, .. } => {
    ...
    fn defer_candidates(&mut self, parent_count: usize) {
        match self {
            Self::Current(current) => current.defer_for_shared_activation(parent_count),
            Self::Or { source, .. } => source.defer_for_shared_activation(parent_count),
        }
    }
}
...
struct FormulaBatch {
    ...
    parents: RowBatch,
    /// Open OR reducers plus an optional, trailing affine stream. Structural
    /// AND ancestry is absent. Cell depth is payload custody only: canonical
    /// PC control decides every result destination.
    cells: Vec<FormulaLiveCell>,
}

impl FormulaBatch {
    fn has_current(&self) -> bool {
        matches!(self.cells.last(), Some(FormulaLiveCell::Current(_)))
    }

    fn current(&self) -> Option<&CandidatePayload> {
        match self.cells.last() {
            Some(FormulaLiveCell::Current(current)) => Some(current),
            Some(FormulaLiveCell::Or { .. }) | None => None,
        }
    }

    fn current_mut(&mut self) -> Option<&mut CandidatePayload> {
    ...
    fn input(&self) -> &CandidatePayload {
        self.current()
            .expect("Formula phase has no affine candidate stream")
    }
    ...
    fn or_count(&self) -> usize {
        self.cells.len() - usize::from(self.has_current())
    }

    fn last_or(&self) -> Option<(&CandidatePayload, &FormulaOrAccumulator)> {
        let index = self
            .cells
            .len()
            .checked_sub(1 + usize::from(self.has_current()))?;
        match &self.cells[index] {
            FormulaLiveCell::Or {
                source,
                accumulator,
            } => Some((source, accumulator)),
            FormulaLiveCell::Current(_) => unreachable!("Current is always the trailing cell"),
        }
    }

    fn last_or_mut(&mut self) -> Option<(&mut CandidatePayload, &mut FormulaOrAccumulator)> {
        let index = self
            .cells
            .len()
            .checked_sub(1 + usize::from(self.has_current()))?;
    ...
    fn open_or(&mut self) {
        ...
        let mut source = self
            .current()
            .is_some()
            .then(|| self.take_current())
            .unwrap_or_else(|| CandidatePayload::empty(parent_count));
        source.defer_for_shared_activation(parent_count);
        self.cells.push(FormulaLiveCell::Or {
            source,
            accumulator: FormulaOrAccumulator::empty(parent_count),
        });
    }
    ...
    fn begin_or_arm(&mut self, stage: FormulaStage) {
        ...
        match stage {
            ...
            // Propose opens its sink transition-locally; there is no affine
            // stream to retain while its Action is planned or running.
    ...
    fn take_current(&mut self) -> CandidatePayload {
        match self.cells.pop() {
            Some(FormulaLiveCell::Current(current)) => current,
            Some(cell @ FormulaLiveCell::Or { .. }) => {
                self.cells.push(cell);
                panic!("Formula phase has no affine candidate stream")
            }
            None => panic!("Formula phase has no affine candidate stream"),
        }
    }
    ...
    fn install_current(&mut self, current: CandidatePayload) {
        ...
        assert!(
            !self.has_current(),
            "Formula phase installed two affine candidate streams"
        );
        self.cells.push(FormulaLiveCell::Current(current));
    }
    ...
    fn from_confirmation(
        ...
        let mut batch = Self {
            ...
            cells: vec![FormulaLiveCell::Current(batch.candidates)],
        };
        if matches!(root_kind, FiniteFormulaNodeKind::Or { .. }) {
            batch.open_or();
        }
    ...
    fn admit_current_or_value(&mut self, parent: u32, value: RawInline) {
        ...
        let accumulator = self
            .last_or_mut()
            .expect("Formula OR admission lost its reducer cell")
            .1;
    ...
    fn current_or_set(&self) -> OrdSet<RawInline> {
        assert!(
            !self.has_current(),
            "Formula OR emitted with a retained arm"
        );
        let accumulator = self
            .last_or()
    ...
    fn install_emitted_or(&mut self, result: CandidatePayload) {
        ...
        assert!(
            matches!(self.cells.pop(), Some(FormulaLiveCell::Or { .. })),
            "emitted Formula OR lost its reducer cell"
        );
    ...
    fn take_contiguous_confirm_original(&mut self) -> CandidatePayload {
        ...
        self.take_confirm_original()
    }

    /// A Formula return can always receive a finalized Confirm result even
    /// though action execution deliberately removes the `Current` cell while
    /// that result is owned by the typed source/program continuation.
    ...
    fn apply_action_result(&mut self, stage: FormulaStage, result: CandidatePayload) {
        ...
        assert!(
            !self.has_current(),
            "a Formula action retained its consumed input"
        );
        self.install_current(result);
    }
    ...
    fn validate_tags(&self) {
        assert!(
            self.cells.iter().all(|cell| match cell {
                FormulaLiveCell::Current(current) => {
    ...
    fn current_is_live(&self) -> Vec<bool> {
        ...
        self.input().mark_live_parents(&mut live);
        live
    }
    ...
    fn admit_current_and_set_tail_stable(&mut self) -> bool {
        let parent_count = self.parents.row_count;
        self.input_mut().admit_set_tail_stable(parent_count)
    }
    ...
    fn append(&mut self, mut other: Self) {
        ...
        assert_eq!(
            self.cells.len(),
            other.cells.len(),
            "one Formula state received incompatible live-cell layouts"
        );
        for (left, right) in self.cells.iter_mut().zip(other.cells) {
            match (left, right) {
                (FormulaLiveCell::Current(left), FormulaLiveCell::Current(right)) => {
                    left.append_disjoint(right, left_parents, right_parents)
                }
                (
                    FormulaLiveCell::Or {
                        source: left_source,
                ...
                _ => panic!("one Formula state received incompatible live-cell shapes"),
            }
        }
    }
    ...
    fn take_tail(&mut self, stride: usize, width: usize) -> Self {
        ...
        let cells = self
            .cells
            .iter_mut()
            .map(|cell| cell.take_parent_tail(first, old_parent_count))
            .collect();
        Self {
            activations,
            parents: RowBatch {
        ...
        K: Clone + Ord,
    {
        ...
        let mut remap = vec![u32::MAX; row_count];
        let mut groups: BTreeMap<K, Self> = BTreeMap::new();
        let empty_cells: Vec<_> = cells.iter().map(|cell| cell.empty_like(0)).collect();
            ...
            K: Clone + Ord,
        {
            ...
            for (parent, value) in pairs {
                ...
                let values = match target
                    .cells
                    .get_mut(cell_index)
                    .expect("partitioned Formula lost a live cell")
                {
                    FormulaLiveCell::Current(current) => current,
                    FormulaLiveCell::Or { source, .. } => source,
                };
        ...
        for (cell_index, cell) in cells.into_iter().enumerate() {
            match cell {
                FormulaLiveCell::Current(current) => {
                    partition_values(current, assignment, &remap, &mut groups, cell_index);
                }
                FormulaLiveCell::Or {
                    source,
                    accumulator,
                } => {
                    partition_values(source, assignment, &remap, &mut groups, cell_index);
                    assert_eq!(accumulator.sets.len(), row_count);
                    for (parent, set) in accumulator.sets.into_iter().enumerate() {
                        let target = groups
                            .get_mut(&assignment[parent])
                            .expect("every Formula assignment created its group");
                        let FormulaLiveCell::Or { accumulator, .. } = &mut target.cells[cell_index]
                        else {
                            panic!("partitioned Formula changed a live-cell shape")
                        };
        ...
        for group in groups.values_mut() {
            ...
            for cell in &mut group.cells {
                cell.normalize_and_defer_source(parent_count);
            }
        }
        groups
    }
    ...
    fn defer_all_candidates(&mut self) {
        ...
        for cell in &mut self.cells {
            cell.defer_candidates(parent_count);
        }
    }
    ...
    fn finish(self) -> CandidateBatch {
        ...
        let Some(FormulaLiveCell::Current(current)) = cells.pop() else {
            panic!("a Formula root finished without a candidate stream")
        };
        assert!(
            cells.is_empty(),
            "a Formula root retained dead payload cells"
        );
        current.debug_assert_valid_for(self.activations.len());
        CandidateBatch {
...
enum FormulaReducerContinuation {
    Complete(FormulaCursor),
    Continue(FormulaCursor),
}
...
impl SelectedResidualTask {
    ...
    fn is_action(&self, interner: &StateInterner) -> bool {
        match &self.desc.phase {
            ...
            ResidualPhase::Formula { cursor } => matches!(
                &interner.formula(*cursor).focus,
                FormulaFocus::Action { .. }
            ),
            ResidualPhase::Ready | ResidualPhase::Candidate { .. } => false,
    ...
    fn action_task(
        ...
        let (action, candidate_occurrences) = match (&self.desc.phase, &self.bucket) {
            ...
            (ResidualPhase::Formula { cursor }, StateBucket::Formula(batch)) => {
                let record = interner.formula(*cursor);
...
fn propose_action_transition<'a>(
    ...
    if plan.has_finite_formula(proposer) {
        ...
        let cursor = interner.start_formula_with_proposer_checked(
            &plan.finite_formula,
            variable,
            proposer,
            UnionVerb::Propose {
...
fn confirm_action_transition<'a>(
    ...
    if plan.has_finite_formula(confirmer) {
        ...
        let formula_root = plan
            ...
            .root(confirmer)
            .expect("a lowered formula has a root");
        let cursor = interner.start_formula(
            &plan.finite_formula,
            variable,
            confirmer,
            UnionVerb::Confirm {
...
fn finish_formula_candidate_transition(
    ...
    let exit = interner.formula_pcs.candidate_exit_by_id(exit).clone();
    let successor = StateDesc {
        bound: desc.bound,
        phase: ResidualPhase::Candidate {
            variable: exit.variable,
            relevant: exit.relevant,
            checked: exit.checked,
        },
    };
    if crosses_candidate_set_boundary(desc, &successor, plan, &interner.formula_pcs) {
    ...
    match successor {
        Ok(InternedFormulaSuccessor::Formula(parent)) => {
            ...
            if decisive {
                ...
                let completed = parent.with_pc(completed);
            ...
            if done_count
                == node
                    ...
                    .len()
            {
                let completed = parent.with_pc(
                    interner
                        .formula_pcs
                        .complete(&plan.finite_formula, parent.pc),
                );
                return propagate_formula_support(
                    plan,
                    desc,
            ...
            file_with_plan(
                ...
                StateDesc {
                    bound: desc.bound,
                    phase: ResidualPhase::Formula { cursor: parent },
                },
        ...
        Ok(InternedFormulaSuccessor::Guard { parent, child }) => {
            let next_pc = if truth {
                interner
                    .formula_pcs
                    .select_supported_child(&plan.finite_formula, parent.pc, child)
            } else {
                interner
                    .formula_pcs
                    .skip_child(&plan.finite_formula, parent.pc, child)
            };
            let next = parent.with_pc(next_pc);
            let mut batch = batch;
            if truth {
...
fn continue_formula_transition(
    ...
    let support_complete = match &interner.formula(cursor).focus {
        FormulaFocus::Plan {
            node,
            stage: FormulaStage::Support,
            done,
    ...
    let confirm_node = match interner.formula(cursor).focus {
        FormulaFocus::Plan {
            node,
    ...
    if let Some(node) = confirm_node {
        if matches!(
            ...
            if !first {
                loop {
                    let child = {
                        let FormulaFocus::Plan { node, done, .. } = &interner.formula(cursor).focus
                        else {
                    ...
                    cursor = cursor.with_pc(interner.formula_pcs.skip_child(
                        &plan.finite_formula,
                        cursor.pc,
                        child,
                    ));
    ...
    let complete = match &interner.formula(cursor).focus {
    ...
    let complete_node = match &interner.formula(cursor).focus {
    ...
    let destination = interner.formula_pcs.result_destination(cursor.pc);
    if destination == FormulaResultDestination::ParentOr {
        let mut batch = batch;
        let input = batch.take_current();
        debug_assert!(!batch.has_current());
        reducer_seeds.push(FormulaReducerSeed::Admit(FormulaOrAdmissionSeed {
            bound: desc.bound,
            batch,
            input,
            continuation: FormulaReducerContinuation::Complete(cursor),
        }));
        return None;
    }
        ...
        .resume_completed(&plan.finite_formula, completed)
    {
        ...
        Err(exit) if destination == FormulaResultDestination::Root => finish_formula_transition(
            plan,
            desc,
            exit,
            batch,
...
fn finish_formula_or_admission(
    ...
    match continuation {
        FormulaReducerContinuation::Complete(cursor) => {
            let completed = cursor.with_pc(
                interner
                    .formula_pcs
                    .complete(&plan.finite_formula, cursor.pc),
                ...
                .resume_completed(&plan.finite_formula, completed)
            {
                ...
                Err(exit) => finish_formula_transition(
                    plan,
                    &desc,
                    exit,
                    batch,
        ...
        FormulaReducerContinuation::Continue(cursor) => {
            let desc = StateDesc {
                bound,
                phase: ResidualPhase::Formula { cursor },
            };
...
fn finish_formula_or_emission(
    ...
    match (successor, destination) {
        ...
        (Err(exit), FormulaResultDestination::Root) => {
            let candidate = batch.finish();
            finish_formula_candidate_transition(
                plan,
                &desc,
...
fn formula_plan_transition<'a>(
    ...
    let (node, stage) = match &interner.formula(cursor).focus {
        FormulaFocus::Plan { node, stage, .. } => (*node, *stage),
...
fn formula_support_plan_transition(
    ...
    let (node, done) = match &interner.formula(cursor).focus {
        FormulaFocus::Plan {
            node,
            stage: FormulaStage::Support,
            done,
    ...
    let next = cursor.with_pc(select_interned_formula_child(
        &plan.finite_formula,
...
fn enter_selected_formula_node(
    ...
    if formula_pcs.result_destination(counter) == FormulaResultDestination::ParentOr {
...
fn formula_or_plan_transition<'a>(
    ...
    let variable = interner.formula_candidate_exit(cursor).variable;
    let (node, done) = match &interner.formula(cursor).focus {
        FormulaFocus::Plan { node, done, .. } => (*node, done.clone()),
    ...
    for (child, batch) in batch.partition(vars.len(), &assignments) {
        let next = cursor.with_pc(interner.formula_pcs.guard_child(
            &plan.finite_formula,
            cursor.pc,
            child,
        ));
...
fn formula_and_plan_transition<'a>(
    ...
    let variable = interner.formula_candidate_exit(cursor).variable;
    let (node, stage, done) = match &interner.formula(cursor).focus {
        FormulaFocus::Plan {
            node, stage, done, ..
        } => (*node, *stage, done.clone()),
...
fn finish_formula_action_result(
    ...
    let stage = match &interner.formula(cursor).focus {
...
fn execute_task<'a>(
    ...
    let stable = match (&desc.phase, bucket) {
        ...
        (ResidualPhase::Formula { cursor }, StateBucket::Formula(batch)) => {
            let cursor = *cursor;
            let action_stage = match &interner.formula(cursor).focus {
...
impl ResidualStateMachine {
    ...
    fn seed_delta_confirm<'a>(
        ...
        .map(|publication| {
            ...
            let Some((support_spec, support_route)) = select_program(constraint, support_request)
            else {
                return publication;
            };
    ...
    fn seed_delta_formula<'a>(
        ...
        let (ResidualPhase::Formula { cursor }, StateBucket::Formula(batch)) =
            (&task.desc.phase, &task.bucket)
        else {
            return Err(task);
        };
        let cursor = *cursor;
        let outer_variable = self.interner.formula_candidate_exit(cursor).variable;
        let (node, stage) = {
            let counter = self.interner.formula(cursor);
            match &counter.focus {
                FormulaFocus::Action { node, stage } => (*node, *stage),
                _ => return Err(task),
            }
...
/// Test-only semantic oracle for the ordinary `Constraint` protocol.
///
/// The wrapper forwards every ordinary relational operation and deliberately
/// hides all residual structure and typed Program capabilities. Running it
/// through the one production compiler therefore exercises the same scheduler
/// with one opaque ordinary-action leaf, without shipping a compiler-policy
/// switch.
#[cfg(test)]
#[derive(Clone)]
struct OrdinaryConstraintOracle<C>(C);

#[cfg(test)]
impl<'a, C: Constraint<'a>> Constraint<'a> for OrdinaryConstraintOracle<C> {
    ...
    fn action_unit_classes(
        ...
        self.0.action_unit_classes(variable, bound)
    }
    ...
    P: Fn(&Binding) -> Option<R>,
{
    ...
    #[cfg(test)]
    fn solve_ordinary_constraint_oracle(
        self,
    ) -> ResidualStateIter<OrdinaryConstraintOracle<C>, P, R> {
        assert_fresh(&self);
        let Query {
            constraint,
            postprocessing,
        ...
        let constraint = OrdinaryConstraintOracle(constraint);
        let full = constraint.variables();
        let plan = ResidualPlan::compile_production(&constraint);
        let state = ResidualStateMachine::new_for_plan(full, &plan, seed);
        ResidualStateIter {
...
mod tests {
    ...
    fn complete_terminal_panic_precedes_quote_prefix_receipts_and_ledger() {
        ...
        let panic_plan = ResidualPlan::compile_production(&panic_root);
        let mut panicking =
    ...
    fn formula_pc_arena_is_exact_compact_and_query_local() {
        ...
        let _exit = arena.intern_candidate_exit(CandidateExit {
            variable: 0,
            relevant: relevant.clone(),
            checked: relevant,
        });
        ...
        assert_eq!(arena.candidate_exit_len(), 1);
        ...
        let legacy_counter = FormulaProgramCounter {
            ...
            exit: CandidateExit {
                ...
                checked: ChildSet::empty(2).with_inserted(0),
            },
        };

        assert_eq!(std::mem::size_of::<FormulaPcId>(), 4);
        assert_eq!(std::mem::size_of::<FormulaCursor>(), 8);
        ...
        eprintln!(
            "formula_pc={} formula_record={} formula_pc_id={} formula_cursor={} state_desc={} legacy_state_desc={} candidate_exits={} returns={} records={}",
            std::mem::size_of::<FormulaProgramCounter>(),
            std::mem::size_of::<FormulaPcRecord>(),
            std::mem::size_of::<FormulaPcId>(),
            std::mem::size_of::<FormulaCursor>(),
            std::mem::size_of::<StateDesc>(),
            std::mem::size_of::<LegacyStateDescLayout>(),
            arena.candidate_exit_len(),
            arena.return_len(),
            arena.len(),
        );
    }
    ...
    fn compiled_formula_proposals_discharge_outer_self_confirm_only_when_sources_do() {
        ...
        assert!(interner.formula_candidate_exit(*cursor).checked.contains(0));
    ...
    fn persistent_formula_pcs_match_structural_oracle_and_reconverge() {
        ...
        let compact_guard =
            compact_start.with_pc(or_arena.guard_child(or_program, compact_start.pc, 0));
        ...
        let compact_guard_complete =
            compact_guard.with_pc(or_arena.complete(or_program, compact_guard.pc));
        ...
        let compact_false = compact_parent.with_pc(or_arena.skip_child(
            or_program,
            compact_parent.pc,
            compact_child,
        ));
        ...
        let compact_true_action = compact_parent.with_pc(or_arena.select_supported_child(
            or_program,
            compact_parent.pc,
            compact_child,
        ));
        ...
        let compact_true_complete =
            compact_true_action.with_pc(or_arena.complete(or_program, compact_true_action.pc));
    ...
    fn finite_formula_support_without_a_program_remains_ordinary() {
        ...
        let support = parent.with_pc(machine.interner.formula_pcs.guard_child(
            &plan.finite_formula,
            parent.pc,
            0,
        ));
        ...
        let task = SelectedResidualTask {
            ...
            desc: StateDesc {
                bound: VariableSet::new_empty(),
                phase: ResidualPhase::Formula { cursor: support },
            },
    ...
    fn formula_payload_installs_ordered_local_or_emission_into_and() {
        ...
        assert_eq!(batch.or_count(), 0);
        assert!(batch.input().is_values());
        assert_eq!(batch.input().one_parent_values(), [raw(1), raw(2)]);
    }
    ...
    fn formula_live_cells_encode_only_payload_that_exists_in_each_phase() {
        ...
        let or = FiniteFormulaNodeKind::Or {
            children: Box::new([]),
        };

        // A proposal has no candidate stream before its selected source
        // returns. Arbitrarily deep AND ancestry remains a payload no-op.
        let mut proposal =
            FormulaBatch::from_proposal(RowBatch::seed(), vec![ActivationId(0)], &and);
        assert!(proposal.cells.is_empty());
        proposal.enter_node(&and);
        proposal.enter_node(&and);
        assert!(proposal.cells.is_empty());
        proposal.apply_action_result(
            FormulaStage::Propose,
            CandidatePayload::Values(vec![raw(1)]),
        );
        assert!(matches!(
            proposal.cells.as_slice(),
            [FormulaLiveCell::Current(_)]
        ));

        // A Confirm action moves its affine input into action/reducer custody.
        // Its saved Formula return therefore carries no Current until the
        // action result is installed.
        let mut confirm = FormulaBatch::from_confirmation(
            CandidateBatch {
                parents: RowBatch::seed(),
                candidates: CandidatePayload::Values(vec![raw(2)]),
            },
            vec![ActivationId(1)],
            &and,
        );
        confirm.enter_node(&and);
        assert!(matches!(
            confirm.cells.as_slice(),
            [FormulaLiveCell::Current(_)]
        ));
        let original = confirm.take_confirm_original();
        assert_eq!(original.one_parent_values(), [raw(2)]);
        assert!(confirm.cells.is_empty());
        confirm.apply_action_result(FormulaStage::Confirm, original);
        assert!(matches!(
            confirm.cells.as_slice(),
            [FormulaLiveCell::Current(_)]
        ));

        // OR owns one irreducible reducer cell. A Confirm arm temporarily
        // appends Current; admission and emission own no current stream; EOF
        // replaces the OR cell with the emitted Current result.
        let mut union = FormulaBatch::from_confirmation(
            CandidateBatch {
                parents: RowBatch::seed(),
                candidates: CandidatePayload::Values(vec![raw(3)]),
            },
            vec![ActivationId(2)],
            &or,
        );
        assert!(matches!(
            union.cells.as_slice(),
            [FormulaLiveCell::Or { .. }]
        ));
        union.begin_or_arm(FormulaStage::Confirm);
        assert!(matches!(
            union.cells.as_slice(),
            [FormulaLiveCell::Or { .. }, FormulaLiveCell::Current(_)]
        ));
        let admitted = union.take_current();
        assert!(matches!(
            union.cells.as_slice(),
            [FormulaLiveCell::Or { .. }]
        ));
        for (parent, value) in admitted.iter() {
            union.admit_current_or_value(parent, value);
        }
        let emitted = CandidatePayload::Values(union.current_or_set().iter().copied().collect());
        union.install_emitted_or(emitted);
        assert!(matches!(
            union.cells.as_slice(),
            [FormulaLiveCell::Current(_)]
        ));
    ...
    fn formula_payload_erases_nested_and_while_preserving_or_action_fanout() {
        ...
        assert_eq!(batch.or_count(), 0, "AND entry allocated payload state");
        assert_eq!(batch.input().one_parent_values(), [raw(3), raw(1), raw(3)]);

        batch.enter_node(&FiniteFormulaNodeKind::Or {
            children: Box::new([]),
        });
        assert_eq!(batch.or_count(), 1);
        ...
        batch.install_emitted_or(emitted);
        assert_eq!(batch.or_count(), 0);
        assert_eq!(batch.input().one_parent_values(), [raw(3)]);
    ...
    fn formula_confirm_opening_keeps_legacy_ropes_structural_but_recoalesces_slice_consumers() {
        ...
        let batch = FormulaBatch {
            activations: vec![ActivationId(0)],
            parents: RowBatch::seed(),
            cells: vec![FormulaLiveCell::Current(segmented)],
        };
        ...
        let structural = legacy.take_confirm_original();
        assert_eq!(
            structural.iter().collect::<Vec<_>>(),
            [(0, raw(1)), (0, raw(2)), (0, raw(1)), (0, raw(3))]
        );
        let CandidatePayload::Deferred(structural) = &structural else {
            panic!("legacy Confirm opening materialized its shared rope")
        };
        assert!(!legacy.has_current());
        assert!(structural.root.is_some());

        let mut slice_consumer = batch;
        let contiguous = slice_consumer.take_contiguous_confirm_original();
        assert_eq!(
            contiguous.one_parent_values(),
            [raw(1), raw(2), raw(1), raw(3)]
        );
        assert!(!slice_consumer.has_current());
    ...
    fn formula_confirm_frame_entry_shares_the_immutable_or_source_root() {
        ...
        let CandidatePayload::Deferred(child) = batch.input() else {
    ...
    fn one_parent_production_and_ordinary_oracle_actions_stay_plain() {
        ...
        for ordinary in [false, true] {
            ...
            let query = Query::new(root(&log), |binding: &Binding| binding.get(0).copied());
            let results: Vec<_> = if ordinary {
                query.solve_ordinary_constraint_oracle().collect()
            } else {
    ...
    fn formula_and_payload_requires_only_parent_grouping() {
        ...
        let _consumed = batch.take_current();
    ...
    fn formula_uniform_partition_reuses_the_complete_payload() {
        let batch = FormulaBatch {
            ...
            cells: vec![
                ...
                FormulaLiveCell::Current(candidate_payload(
                    3,
        ...
        let FormulaLiveCell::Or {
            ...
            panic!("uniform partition changed its OR cell")
        };
        ...
        assert_eq!(
            group.input().iter().collect::<Vec<_>>(),
            vec![(0, raw(30)), (1, raw(31)), (2, raw(32))]
        );
        ...
        let empty = FormulaBatch {
            ...
            cells: vec![FormulaLiveCell::Current(CandidatePayload::Tagged(
                Vec::new(),
            ))],
        };
    ...
    fn finite_formula_compiler_grades_every_adaptive_transition_strictly() {
        ...
        loop {
            ...
            assert_eq!(formula_pcs.grade(compact.pc), grade);
            let (successor, compact_successor) = match &counter.focus {
                FormulaFocus::Action { .. } => (
                    ...
                    Ok(InternedFormulaSuccessor::Formula(
                        compact.with_pc(formula_pcs.complete(program, compact.pc)),
                    )),
                ),
                FormulaFocus::Plan { node, done, .. } => {
                    ...
                    if done.count() == children.len() {
                        ...
                        (
                            ...
                            Ok(InternedFormulaSuccessor::Formula(compact.with_pc(
                                formula_pcs.select_child(program, compact.pc, child),
                            ))),
                        )
                    }
                }
            ...
            match (successor, compact_successor) {
                ...
                (FormulaSuccessor::Outer(exit), Err(compact_exit)) => {
                    assert_eq!(exit, counter.exit);
                    assert_eq!(formula_pcs.candidate_exit_by_id(compact_exit), &exit);
    ...
    fn finite_formula_ranks_fit_strictly_between_outer_protocol_grades() {
        ...
        let action = start.with_pc(formula_pcs.select_child_as_action(program, start.pc, 0));
        let child_complete = action.with_pc(formula_pcs.complete(program, action.pc));
        ...
        let second_action =
            next_plan.with_pc(formula_pcs.select_child_as_action(program, next_plan.pc, 1));
        let second_complete =
            second_action.with_pc(formula_pcs.complete(program, second_action.pc));
        ...
        let root_complete = full_plan.with_pc(formula_pcs.complete(program, full_plan.pc));
        ...
        .map(|cursor| rank(&formula_pcs, ResidualPhase::Formula { cursor }));
        ...
        // An Exact proposal already names the next checked Candidate band.
        // Its Formula control follows Candidate{} but remains strictly before
        // the exact Candidate{owner} future it will materialize.
        let exact_start = formula_pcs.start_with_proposer_checked(
            program,
            0,
            1,
            UnionVerb::Propose {
        ...
        let exact_start_rank = rank(
            &formula_pcs,
            ResidualPhase::Formula {
                cursor: exact_start,
            },
        );
        let exact_checked = ChildSet::empty(plan.len()).with_inserted(1);
        let exact_candidate = rank(
            &formula_pcs,
            ResidualPhase::Candidate {
                variable: 0,
                relevant: relevant.clone(),
                checked: exact_checked,
            },
        );
        assert!(exact_start_rank > proposal_candidate);
        assert!(exact_start_rank < exact_candidate);
    ...
    fn continuation_publication_receipt_barriers_activation_reuse_and_control_work() {
        ...
        let formula_plan = ResidualPlan::compile_production(&formula_root);
        let mut formula_pcs = FormulaPcInterner::default();
    ...
    fn rpq_program_is_total_for_both_partial_confirm_directions_and_formula_support() {
        ...
        let mut supported_query = Query::new(support_root, project).solve_residual_state_lazy();
    ...
    fn typed_terminal_rpq_matches_sparse_and_ordinary_oracle_across_path_shapes() {
        ...
        for (name, ops) in cases {
            let mut baseline: Vec<_> = Query::new_projected(make(&ops), [0, 2], project)
                .solve_ordinary_constraint_oracle()
                .collect();
    ...
    #[test]
    fn online_formula_pages_exact_typed_proposals() {
        let proposes = Arc::new(AtomicUsize::new(0));
        let pages = Arc::new(AtomicUsize::new(0));
    ...
    fn recursive_union_compiler_flattens_or_but_preserves_and_nodes() {
        ...
        let plan = ResidualPlan::compile_production(&make());
        let formula_root = plan.finite_formula.root(0).unwrap();
    ...
    #[cfg(feature = "parallel")]
    #[test]
    fn parallel_residual_preserves_the_production_transition_program() {
        use crate::id::{id_into_value, ExclusiveId, Id};
        use crate::query::regularpathconstraint::{PathOp, RegularPathConstraint};
        use crate::trible::{Trible, TribleSet};
        ...
        let ordinary_counters = program_fallback_counters();
        ...
        assert!(
            ordinary_counters.0.load(Ordering::Relaxed) > 0,
            "the opaque oracle must use the ordinary RPQ proposal path"
        );

...
/// Bounded parallelism for provider-directory replica fan-out.
const MAX_CONCURRENT_PROVIDER_RPCS: usize = ALPHA;
/// Bound exact-key publication independently of the alpha-bounded RPC fan-out
/// inside one DHT announcement. The ordered scheduler carries fairness across
/// arbitrary publication-set sizes.
const MAX_CONCURRENT_PROVIDER_ANNOUNCEMENTS: usize = ALPHA;
...
const PROVIDER_BACKLOG_WARNING_INTERVAL: std::time::Duration = std::time::Duration::from_secs(60);
...
/// One exact-key DHT fan-out is bounded independently of the host scheduler.
const PROVIDER_PUBLICATION_DEADLINE: std::time::Duration = std::time::Duration::from_secs(45);
...
struct ProviderAnnouncementOutcome {
    key: ProviderKey,
...
/// Bounded fair scheduling over the exact keys selected for publication.
/// Unchanged keys retain their deadlines; newly selected keys publish
...
struct ProviderSetScheduler {
    ...
    lease_deadline_by_key: HashMap<ProviderKey, crate::clock::Mono>,
    in_flight: HashSet<ProviderKey>,
    failures: HashMap<ProviderKey, u32>,
    next_backlog_warning: Option<crate::clock::Mono>,
}

impl ProviderSetScheduler {
    fn new() -> Self {
        Self {
            ...
            lease_deadline_by_key: HashMap::new(),
    ...
    fn observe_set(&mut self, set: ProviderSet, now: crate::clock::Mono) {
        ...
        let removed: Vec<_> = self.set.iter().filter(|key| !set.contains(key)).collect();
        for key in removed {
            self.unschedule(key);
            self.remove_lease_deadline(key);
            self.failures.remove(&key);
        }

        let added: Vec<_> = set.iter().filter(|key| !self.set.contains(key)).collect();
        self.set = set;
        for key in added {
            self.failures.remove(&key);
    ...
    fn schedule(&mut self, key: ProviderKey, due: crate::clock::Mono) {
        if let Some(previous) = self.due_by_key.insert(key, due) {
            self.due.remove(&(previous, key));
        }
        self.due.insert((due, key));
    }

    fn unschedule(&mut self, key: ProviderKey) {
        if let Some(previous) = self.due_by_key.remove(&key) {
            self.due.remove(&(previous, key));
        }
    }

    fn set_lease_deadline(&mut self, key: ProviderKey, deadline: crate::clock::Mono) {
        if let Some(previous) = self.lease_deadline_by_key.insert(key, deadline) {
            self.lease_deadlines.remove(&(previous, key));
        }
        self.lease_deadlines.insert((deadline, key));
    }

    fn remove_lease_deadline(&mut self, key: ProviderKey) {
        if let Some(previous) = self.lease_deadline_by_key.remove(&key) {
            self.lease_deadlines.remove(&(previous, key));
        }
    }

    fn pop_due(&mut self, now: crate::clock::Mono) -> Option<ProviderKey> {
        loop {
            let (due, key) = self.due.first().copied()?;
            if due > now {
                return None;
            }
    ...
    fn complete(&mut self, outcome: ProviderAnnouncementOutcome, now: crate::clock::Mono) {
        self.in_flight.remove(&outcome.key);
        if !self.set.contains(&outcome.key) {
            return;
        }
        ...
        let delay = if outcome.publication.succeeded() {
            ...
            self.set_lease_deadline(outcome.key, outcome.attempted_at + PROVIDER_LEASE_LIFETIME);
    ...
    fn warnable_expired_lease(
        &mut self,
        now: crate::clock::Mono,
    ) -> Option<(ProviderKey, crate::clock::Mono)> {
        let (deadline, key) = self.lease_deadlines.first().copied()?;
        if deadline > now
            || self
        ...
        self.next_backlog_warning = Some(now + PROVIDER_BACKLOG_WARNING_INTERVAL);
        Some((key, deadline))
    }
    ...
    #[cfg(test)]
    fn next_due(&self, key: ProviderKey) -> Option<crate::clock::Mono> {
        self.due_by_key.get(&key).copied()
    }

    #[cfg(test)]
    fn lease_deadline(&self, key: ProviderKey) -> Option<crate::clock::Mono> {
        self.lease_deadline_by_key.get(&key).copied()
    }
}
...
async fn host_loop<T: Transport>(harness: Harness<T>, config: PeerConfig, wiring: HostWiring) {
    ...
    loop {
        ...
        while let Ok(outcome) = provider_rx.try_recv() {
            provider_scheduler.complete(outcome, crate::clock::mono_now());
        }
        ...
        if let Some((key, deadline)) = provider_scheduler.warnable_expired_lease(now) {
            warn!(
                ...
                "provider publication backlog missed an exact lease renewal deadline"
            );
        }
        ...
        if config.qos.direction.serves() && snapshot.lock().unwrap().is_some() {
            while provider_scheduler.in_flight_len() < MAX_CONCURRENT_PROVIDER_ANNOUNCEMENTS {
                let Some(key) = provider_scheduler.pop_due(now) else {
                    break;
                };
                ...
                tokio::spawn(async move {
                    ...
                    let publication = tokio::time::timeout(
                        ...
                        client.announce_key(key, remote_expected),
                    )
                    .await
                    ...
                    let _ = provider_tx.send(ProviderAnnouncementOutcome {
                        key,
                        attempted_at,
                        publication,
                    });
...
impl<T: Transport> ProviderClient<T> {
    ...
    async fn announce_key(&self, key: ProviderKey, remote_expected: bool) -> ProviderPublication {
        let targets = self.lookup_replicas(key).await;
        self.announce(key, targets, remote_expected).await
    }
    ...
    async fn announce(
        ...
        let mut replies = futures::stream::iter(targets)
            .map(|target| async move {
                (
                    target,
                    tokio::time::timeout(OP_DEADLINE, self.put(target, key)).await,
                )
            })
        ...
        let _ = tokio::time::timeout(PROVIDER_PUBLICATION_DEADLINE, async {
    ...
    async fn put(&self, target: PeerId, key: ProviderKey) -> bool {
        ...
        match tokio::time::timeout(
            PROVIDER_CONTROL_RPC_DEADLINE,
            op_provider_put(connection.conn(), &key),
        )
        .await
        {
            Ok(Ok(stored)) => stored,
    ...
    async fn serve_stream<C: Conn>(
        ...
        match op {
            ...
            OP_PROVIDER_PUT => {
                ...
                // peer. The body contains one opaque exact rendezvous key.
...
mod tests {
    ...
    fn provider_set(count: usize) -> ProviderSet {
        let set = ProviderSet::from_artifacts((0..count as u64).map(artifact));
    ...
    #[test]
    fn provider_set_scheduler_is_bounded_and_fair_beyond_concurrency() {
        let now = crate::clock::mono_now();
        let count = 3 * MAX_CONCURRENT_PROVIDER_ANNOUNCEMENTS + 2;
        let set = provider_set(count);
        let mut scheduler = ProviderSetScheduler::new();
        scheduler.observe_set(set, now);
        assert_eq!(scheduler.active_len(), count);

        let mut seen = BTreeSet::new();
        while seen.len() < count {
            let mut batch = Vec::new();
            while scheduler.in_flight_len() < MAX_CONCURRENT_PROVIDER_ANNOUNCEMENTS {
                let Some(key) = scheduler.pop_due(now) else {
                    break;
                };
                assert!(seen.insert(key), "an active key launches once per due time");
                batch.push(key);
            }
            assert!(!batch.is_empty());
            assert!(batch.len() <= MAX_CONCURRENT_PROVIDER_ANNOUNCEMENTS);
            for key in batch {
                scheduler.complete(
    ...
    #[test]
    fn provider_set_scheduler_renews_at_half_life_and_retries_with_backoff() {
        let now = crate::clock::mono_now();
        let set = provider_set(2);
        ...
        let success_key = scheduler.pop_due(now).unwrap();
        assert_eq!(success_key, success);
        scheduler.complete(
            ProviderAnnouncementOutcome {
                key: success,
                attempted_at: now,
        ...
        let retry_key = scheduler.pop_due(now).unwrap();
        assert_eq!(retry_key, retry);
        scheduler.complete(
            ProviderAnnouncementOutcome {
                key: retry,
                attempted_at: now,
        ...
        let retry_key = scheduler.pop_due(retry_at).unwrap();
        assert_eq!(retry_key, retry);
        scheduler.complete(
            ProviderAnnouncementOutcome {
                key: retry,
    ...
    fn provider_set_scheduler_preserves_deadlines_across_equivalent_snapshots() {
        ...
        let mut scheduler = ProviderSetScheduler::new();
        scheduler.observe_set(set.clone(), now);
        assert_eq!(scheduler.pop_due(now), Some(key));
        scheduler.complete(
            ProviderAnnouncementOutcome {
                key,
                attempted_at: now,
        ...
        let renewal = scheduler.next_due(key);
        scheduler.observe_set(set, now + Duration::from_secs(1));
        assert_eq!(scheduler.next_due(key), renewal);

        scheduler.observe_set(ProviderSet::default(), now + Duration::from_secs(2));
    ...
    #[test]
    fn removed_in_flight_key_is_not_rescheduled_by_late_completion() {
        let now = crate::clock::mono_now();
        let original = provider_set(1);
        let removed = original.iter().next().unwrap();
        let replacement = ProviderSet::from_artifacts([artifact(1)]);
        assert!(!replacement.contains(&removed));
        let mut scheduler = ProviderSetScheduler::new();
        scheduler.observe_set(original, now);
        assert_eq!(scheduler.pop_due(now), Some(removed));
        scheduler.observe_set(replacement.clone(), now);
        scheduler.complete(
            ProviderAnnouncementOutcome {
                key: removed,
                attempted_at: now,
        ...
        assert_eq!(scheduler.next_due(removed), None);
        assert_eq!(scheduler.lease_deadline(removed), None);
        assert_eq!(scheduler.pop_due(now), replacement.iter().next());
    }

    #[test]
    fn keys_with_one_first_byte_remain_independent_schedule_units() {
        let now = crate::clock::mono_now();
        let mut by_prefix = HashMap::<u8, (ArtifactId, ProviderKey)>::new();
        let pair = (0..u64::MAX)
            .find_map(|index| {
                let artifact = artifact(index);
                let key = provider_key(artifact);
                by_prefix
                    .insert(key[0], (artifact, key))
                    .map(|previous| (previous, (artifact, key)))
            })
            .expect("two provider keys eventually share a first byte");
        assert_eq!(pair.0.1[0], pair.1.1[0]);
        let set = ProviderSet::from_artifacts([pair.0.0, pair.1.0]);
        let mut scheduler = ProviderSetScheduler::new();
        scheduler.observe_set(set, now);
    ...
    #[test]
    fn provider_set_scheduler_rate_limits_definite_lease_miss_warnings() {
        let now = crate::clock::mono_now();
        let set = provider_set(1);
        let key = set.iter().next().unwrap();
        let mut scheduler = ProviderSetScheduler::new();

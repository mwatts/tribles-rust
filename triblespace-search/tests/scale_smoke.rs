...
fn succinct_bm25_1k_docs_matches_naive() {
    ...
    for term_text in ["w0", "w1", "w7", "w50", "w250", "w499"] {
        ...
        // Both builders assign document codes in canonical raw-key order, so
        // posting order and exact scores agree directly.
        ...
        for ((id_a, s_a), (id_b, s_b)) in a.iter().zip(b.iter()) {
            assert_eq!(id_a, id_b);
            assert_eq!(s_a.to_bits(), s_b.to_bits(), "term {term_text}");
        }
    }

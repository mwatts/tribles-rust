...
- **Parallel SimpleArchive decoding builds each worker chunk bottom-up.** For
  ...
  and every bottom-up-built archive Branch retains the shared owner so later
  same-archive unions preserve the nearest-ancestor lifetime invariant. Small,
  unaligned, and oversized inputs keep their existing serial or heap-leaf
  fallbacks.
